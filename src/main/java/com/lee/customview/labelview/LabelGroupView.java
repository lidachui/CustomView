package com.lee.customview.labelview;
...
/**
 * Created by LeeJing on 2018/4/11.
 * 标签组view
 */
public class LabelGroupView extends ViewGroup implements View.OnClickListener {
	...
	public LabelGroupView(Context context, AttributeSet attrs) {
		this(context, attrs, 0);
	}
	
	public LabelGroupView(Context context, AttributeSet attrs, int defStyleAttr) {
		super(context, attrs, defStyleAttr);
		mContext = context;
		if (attrs != null) {
			TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.labels_view);
			int type = typedArray.getInt(R.styleable.labels_view_selectType, 1);
			mSelectType = SelectType.get(type);
			mMaxSelect = typedArray.getInteger(R.styleable.labels_view_maxSelect, 0);
			mTextColor = typedArray.getColorStateList(R.styleable.labels_view_labelTextColor);
			mTextSize = typedArray.getDimension(R.styleable.labels_view_labelTextSize,
					DesityUtil.dip2px(context, 14));
			mTextPaddingLeft = typedArray.getDimensionPixelOffset(
					R.styleable.labels_view_labelTextPaddingLeft, 0);
			mTextPaddingTop = typedArray.getDimensionPixelOffset(
					R.styleable.labels_view_labelTextPaddingTop, 0);
			mTextPaddingRight = typedArray.getDimensionPixelOffset(
					R.styleable.labels_view_labelTextPaddingRight, 0);
			mTextPaddingBottom = typedArray.getDimensionPixelOffset(
					R.styleable.labels_view_labelTextPaddingBottom, 0);
			mLineMargin = typedArray.getDimensionPixelOffset(R.styleable.labels_view_lineMargin, 0);
			mWordMargin = typedArray.getDimensionPixelOffset(R.styleable.labels_view_wordMargin, 0);
			mLabelBgResId = typedArray.getResourceId(R.styleable.labels_view_labelBackground, 0);
			typedArray.recycle();
		}
	}
